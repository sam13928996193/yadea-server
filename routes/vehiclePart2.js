...
router.post('/option',async(req, res, next)=>{
    let {username} = req.user;
    let {sortID,vehicleSeriesID,vehicleModelID,vehicleConfigID,partCategoryID,partID,remark} = req.body;
 
    let usersql = "select id from user where username=?"
    let sql = "insert into vehiclepart(createUserID,lastUserID,sortID,vehicleSeriesID,vehicleModelID,vehicleConfigID,partCategoryID,partID,remark) value(?,?,?,?,?,?,?,?,?)";
    try{
        let user = await querySql(usersql,username);
        let createUserID=user[0].id;

        await querySql(sql,[createUserID,createUserID,sortID,vehicleSeriesID,vehicleModelID,vehicleConfigID,partCategoryID,partID,remark]); 
...
router.get('/option',async(req, res, next)=>{
    let id=req.query.id;
    let sql = "select id,status,createUserID,createDate,lastUserID,lastDate,sortID,vehicleSeriesID,vehicleModelID,vehicleConfigID,partCategoryID,partID,remark,(select vehicleSeriesName from vehicleseries where ID=a.vehicleSeriesID) as vehicleSeriesName ,(select vehicleSeriesCode from vehicleseries where ID=a.vehicleSeriesID) as vehicleSeriesCode ,(select vehicleModelName  from vehiclemodel  where ID=a.vehicleModelID) as vehicleModelName ,(select vehicleModelCode from vehiclemodel  where ID=a.vehicleModelID) as vehicleModelCode ,(select vehicleConfigCode from vehicleconfig where ID=a.vehicleConfigID) as vehicleConfigCode,(select vehicleConfigName from vehicleconfig where ID=a.vehicleConfigID) as vehicleConfigName,(select partCategoryCode from partcategory where ID=a.partcategoryID) as artCategoryCode ,(select partCategoryName from partcategory where ID=a.partCategoryID) as partCategoryName ,(select partName from part  where ID=a.partID) as partName ,(select partCode from part  where ID=a.partID) as partCode ,(select checkID from part where ID=a.partID) as checkID,(select programID from part where ID=a.partID) as programID,(select fileCode from partsoftware w,part p where w.partID=p.ID and p.ID=a.partID and p.status='2'  LIMIT 1) as file_code,(select fileName from partsoftware w,part p where w.partID=p.ID and p.ID=a.partID and p.status='2'  LIMIT 1) as file_Name,(select  fileVersion  from partsoftware w,part p where w.partID=p.ID and p.ID=a.partID and p.status='1'  LIMIT 1) as fileVersion,(select fileUrl from partsoftware w,part p where w.partID=p.ID and p.ID=a.partID and p.status='2') as fileUrl from vehiclepart a where deleted=0 and id=?";
    try{
...
router.put('/option',async(req, res, next)=>{
    ...
    let {username} = req.user;
    let usersql = "select id from user where username=?";
    let {id,sortID,vehicleSeriesID,vehicleModelID,vehicleConfigID,partCategoryID,partID,status,remark}=req.body;
    let sql = "update vehiclepart set lastUserid=?,lastDate=?,sortID=?,vehicleSeriesID=?,vehicleModelID=?,vehicleConfigID=?,partCategoryID=?,partID=?,status=?,remark=?  where id=?";

    try{
        let user = await querySql(usersql,username);
        createUserID=user[0].id;
        let recordData = await querySql(sql,[createUserID,lastDate,sortID,sortID,vehicleSeriesID,vehicleModelID,vehicleConfigID,partCategoryID,partID,status,remark,id]); 
