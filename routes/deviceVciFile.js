...
router.post('/search', async(req, res, next)=>{
    ...
    let sql ="select (select count(id) from deviceVciFile where deleted=0) as total,id,status,createUserID,createDate,lastUserID,lastDate,fileCode,fileName,fileUrl,fileVersion,remark from deviceVciFile where 1=1 and deleted=0" ;
        let arr=[];
        if(fileVersion!=""){
...
router.post('/option',async(req, res, next)=>{
    ...
    let sql = "insert into deviceVciFile(createUserID,createDate,lastUserID,fileCode,fileName,fileUrl,fileVersion,remark) value(?,?,?,?,?,?,?,?)";
    try{
...
router.get('/option',async(req, res, next)=>{
    let id=req.query.id;
    let sql = "select id,status,createUserID,createDate,fileCode,fileName,fileUrl,fileVersion,remark from deviceVciFile where deleted=0 and id=?";
    try{
...
router.put('/option',async(req, res, next)=>{
    ...
    let sql = "update deviceVciFile set lastUserid=?,lastDate=?,fileCode=?,fileName=?,fileUrl=?,fileVersion=?,remark=?  where id=?";
...
router.delete('/option',async(req, res, next)=>{
    ...
    let id=req.query.id;
    let sql = "update deviceVciFile set deleted=1 ,lastUserId=?,lastDate=? where id=?";
