...
router.post('/option',async(req, res, next)=>{
    ...
    let sql = "insert into partsoftware(createUserID,lastUserID,partCategoryID,partID,fileCode,fileName,fileUrl,fileVersion,remark) value(?,?,?,?,?,?,?,?,?)";
    try{
...
router.get('/option',async(req, res, next)=>{
    let id=req.query.id;
    let sql = "select (select count(id) from partsoftware where deleted=0) as total,id,status,createUserID,createDate,lastUserID,lastDate,partCategoryID,(select partCategoryName from Partcategory where ID=a.partCategoryID) as partCategoryName,partID,(select partCode from Part where ID=a.partID) as partCode,(select partName from Part where ID=a.partID) as partName,(select checkID from Part where ID=a.partID) as checkID,(select programID from Part where ID=a.partID) as programID,fileCode,fileName,fileUrl,fileVersion,remark from partsoftware a where deleted=0 and id=?";
    try{
...
router.put('/option',async(req, res, next)=>{
    ...
    let sql = "update partsoftware set lastUserid=?,lastDate=?,partCategoryID=?,partID=?,fileCode=?,fileName=?,status=?,fileUrl=?,fileVersion=?,remark=?  where id=?";
...
router.delete('/option',async(req, res, next)=>{
    ...
    let id=req.query.id;
    let sql = "update partsoftware set deleted=1 ,lastUserId=?,lastDate=? where id=?";
